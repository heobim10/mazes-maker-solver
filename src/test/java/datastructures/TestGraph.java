...
public class TestGraph extends BaseTest {
    ...
    @Test(timeout=SECOND)
    public void testCustomFindingDisconnectedGraphSameStartAndEnd() {
        IList<String> vertices = new DoubleLinkedList<>();
        IList<SimpleEdge> edges = new DoubleLinkedList<>();
        vertices.add("a");
        Graph<String, SimpleEdge> graph = new Graph<String, SimpleEdge>(vertices, edges);
        IList<SimpleEdge> path = graph.findShortestPathBetween("a", "a");
        assertEquals(0, path.size());
    }
    ...
    public void testCustomExceptions() {
        ...
        // Empty graphs are technically fine.
        new Graph<>(vertices, edges);

        // Null graphs are no good.
        vertices.add("a");
        vertices.add("b");
        vertices.add("c");
        vertices.add(null);
        ...
        // Phantom edges are no good
        edges.add(edge("a", "Undone - The Sweater Song", 3));
        try {
            new Graph<>(vertices, edges);
            fail();
        } catch (IllegalArgumentException e) {
        ...
        // Duplicate vertices are no good.
        vertices.add("c");
        try {
            new Graph<>(vertices, edges);
            fail();
    ...
    public void testCustomMst() {
        ...
        vertices.add("c");
        vertices.add("d");
        edges.add(edge("d", "d", 0));
        edges.add(edge("c", "d", 10000));
        edges.add(edge("d", "b", 2));
        edges.add(edge("d", "c", 10000));
        edges.add(edge("c", "a", 1));
        edges.add(edge("a", "b", 6));
        edges.add(edge("a", "b", 7));
        edges.add(edge("a", "b", 5));

        Graph<String, SimpleEdge> graph = new Graph<>(vertices, edges);
        ISet<SimpleEdge> result = graph.findMinimumSpanningTree();
        assertEquals(3, result.size());
        assertFalse(result.contains(edges.get(0)));
        assertTrue(result.contains(edges.get(2)));
        assertTrue(result.contains(edges.get(4)));
        assertTrue(result.contains(edges.get(7)));
    }
}
